...
type Props = {
  params: Promise<{ locale: string; slug: string; assetId: string }>;
};
...
export async function generateMetadata({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'booking.slotBased' });
...
export default async function BookingPage({ params }: Props) {
  const { locale, slug, assetId } = await params;
  ...
  return <BookingFlow venue={venue} asset={asset} locale={locale} />;
}
